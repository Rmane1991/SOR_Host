...
{
    public partial class AggVerification : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                        bool HasPagePermission = UserPermissions.IsPageAccessibleToUser(Session["UserName"].ToString(), Session["UserRoleID"].ToString(), "AggVerification.aspx", "15");
                    ...
                    UserPermissions.RegisterStartupScriptForNavigationListActive("5", "15");
                      }
                }
                else
                {
            ...
            catch (Exception Ex)
            {
                ...
                ////_dbAccess.StoreErrorDescription(UserName, "AggVerification.cs", "Page_Load()", Ex);

                ScriptManager.RegisterStartupScript(this, typeof(Page),  "Warning", "showWarning('Contact System Administrator', 'Verification');", true);
                return;
            }
        }
        #endregion
        ...
        public DataSet FillGrid(EnumCollection.EnumPermissionType _EnumPermissionType)
        {
            ...
            try
            {
                ...
                _BCEntity.CHstatus = Convert.ToString((int)EnumCollection.Onboarding.CheckerApprove);
                _BCEntity.ATStatus = Convert.ToString((int)EnumCollection.Onboarding.AuthorizerPending);
                ds = _BCEntity.GetAggDetailsToProcessOnboaring();
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                            ...
                            for (int i = 0; i < BCDataSet.Rows.Count; i++)
                            {
                                try
                                {
                                    ...
                                    if (_ActivityType == 0)
                                    {
                                        ...
                                        DataSet dsBCMaster = _BCEntity.SetInsertUpdateAggMasterDetails();
                                        if (dsBCMaster != null && dsBCMaster.Tables.Count > 0 && dsBCMaster.Tables[1].Rows[0]["Status"].ToString() == "Inserted")
                                        {
                                            _dsVerification = _BCEntity.ChangeAggregatorStatus();
                                            ScriptManager.RegisterStartupScript(this, typeof(Page),  "Warning", "showWarning('Data Registered Successfully', 'Verification');", true);
                                        }
                                    }
                                    if (_ActivityType == 1)
                                    {
                                    ...
                                    if (_ActivityType == 4)
                                    {
                                        ...
                                        _dsActivate = _BCEntity.ChangeAggStatusReEdit();
                                        if (_dsActivate != null && _dsActivate.Tables.Count > 0 && _dsActivate.Tables[0].Rows[0][0].ToString() == "1")
                                        {
                                            _dsVerification = _BCEntity.ChangeAggregatorStatus();
                                        }
        ...
        public void SingleSave(int _reocrdsProcessed, int _StatusFlag, int _Flag, string _BCReqId, string _strBCFullName, string User, string Remarks, string ReceiverEmailID, string ContactNo, string _ClientCode, int _ActivityType, string _BCCode)
        {
            try
            {
                ErrorLog.BCManagementTrace("Verification Level: SingleSave: Received - Maker Verification.  StatusFlag: " + ViewState["ActionType"].ToString() + " Flag: " + _Flag + " AgentCode: " + _BCReqId + " Fullname: " + _strBCFullName + " User: " + User + " ReceiverEmailID: " + ReceiverEmailID + " ContactNo: " + ContactNo + " Remarks: " + Remarks);
                _BCEntity.ActivityType = _ActivityType;
                ...
                if (ViewState["ActionType"].ToString() == EnumCollection.EnumDBOperationType.Decline.ToString())
                {
                    _BCEntity.CHstatus = Convert.ToString((int)EnumCollection.Onboarding.CheckerApprove);
                    _dsVerification = _BCEntity.ChangeAggregatorStatus();
                    ScriptManager.RegisterStartupScript(this, typeof(Page),  "Warning", "showSuccess('Request Declined Successfully', 'Verification');", true);
                    ErrorLog.BCManagementTrace("Aggregator-Verification: SingleSave: Successful - Authorizer Verification.  StatusFlag: " + ViewState["ActionType"].ToString() + " Flag: " + _Flag + " AgentCode: " + _BCReqId + " Fullname: " + _strBCFullName + " User: " + User + " ReceiverEmailID: " + ReceiverEmailID + " ContactNo: " + ContactNo + " Remarks: " + Remarks);
                }
                else
                {
                    if (_ActivityType == 0)
                    {
                        ...
                        if (dsBCMaster != null && dsBCMaster.Tables.Count > 0 && status == "Inserted")
                        {
                            ...
                            _BCEntity.CHstatus = Convert.ToString((int)EnumCollection.Onboarding.CheckerApprove);
                            _dsVerification = _BCEntity.ChangeAggregatorStatus();
                            ScriptManager.RegisterStartupScript(this, typeof(Page),  "Warning", "showSuccess('Data Registered Successfully', 'Verification');", true);
                            ErrorLog.BCManagementTrace("Aggregator-Verification: SingleSave: Successful - Authorizer Verification.  StatusFlag: " + ViewState["ActionType"].ToString() + " Flag: " + _Flag + " AgentCode: " + _BCReqId + " Fullname: " + _strBCFullName + " User: " + User + " ReceiverEmailID: " + ReceiverEmailID + " ContactNo: " + ContactNo + " Remarks: " + Remarks + "ActivityType: " + _ActivityType);
                    ...
                    if (_ActivityType == 1)
                    {
                        ...
                        if (_dsActivate != null && _dsActivate.Tables.Count > 0 && _dsActivate.Tables[0].Rows[0][0].ToString() == "1")
                        {
                            ...
                            _dsVerification = _BCEntity.ChangeAggregatorStatus();
                            ScriptManager.RegisterStartupScript(this, typeof(Page),  "Warning", "showSuccess('Request Approved Successfully', 'Verification');", true);
                            ErrorLog.BCManagementTrace("Aggregator-Verification: SingleSave: Successful - Authorizer Verification.  StatusFlag: " + ViewState["ActionType"].ToString() + " Flag: " + _Flag + " AgentCode: " + _BCReqId + " Fullname: " + _strBCFullName + " User: " + User + " ReceiverEmailID: " + ReceiverEmailID + " ContactNo: " + ContactNo + " Remarks: " + Remarks + "ActivityType: " + _ActivityType);
            ...
            {
                ErrorLog.BCManagementTrace("Aggregator-Verification: SingleSave: Failed - Authorizer Verification. Exception: " + Ex.Message);
        ...
        protected void btnExportCSV_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    ScriptManager.RegisterStartupScript(this, typeof(Page),  "Warning", "showWarning('No data found.', 'Verification Detailss');", true);
                }
            }
            catch (Exception Ex)
            {
        ...
        protected void btnExportExcel_ServerClick1(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    ScriptManager.RegisterStartupScript(this, typeof(Page),  "Warning", "showWarning('No data found.', 'Verification Details');", true);
                }
            }
            catch (Exception Ex)
            {
        ...
        protected void btnView_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                DataSet ds = _BCEntity.GetAggDocuments();

                _BCEntity.Mode = "GetBCDocumentByID";
                DataSet get = _BCEntity.GetAggDocuments();
        ...
        protected void ddlState_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                ErrorLog.BCManagementTrace("Page : AggVerification.cs \nFunction : ddlState_SelectedIndexChanged\nException Occured\n" + ex.Message);
