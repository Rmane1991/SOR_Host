...
{
    public partial class ReprocessAggDetails : System.Web.UI.Page
    {
        ...
        public void GetDetails(string BCReqId)
        {
            try
            {
                ...
                DataSet ds = _BCEntity.GetOnboradingAggDetails();
                ...
                if (ds != null && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    //}
                    txtFirstName.Text = ds.Tables[0].Rows[0]["aggName"].ToString();
                    ...
                    if (ds.Tables[0].Rows[0]["aggCategory"].ToString() != "")
                    {
                        ddlCategory.SelectedValue = ds.Tables[0].Rows[0]["aggCategory"].ToString();
                    }
                   ...
                   // txtPass.Text = ds.Tables[0].Rows[0]["PassportNo"].ToString();
                    ...
                    txtRegisteredAddress.Text = ds.Tables[0].Rows[0]["aggAddress"].ToString();
            ...
            try
            {
                if (HiddenField1.Value.ToString() == "Yes")
                {
                    ...
                    if (ValidateSetProperties())
                    {
                        ...
                        {
                            ErrorLog.AgentManagementTrace("EditRegistationDetails: btnSubmitDetails_Click: Failed - Aggregator Registration Request Dump In DB. UserName: " + UserName + " Status: " + Status + " StatusMsg: " + StatusMsg + " RequestId: " + RequestId);
        ...
        protected void BtnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                if (HiddenField1.Value.ToString() == "Yes")
                {
                    ...
                    if (_BCEntity.Insert_aggregatorRequest(Convert.ToString(Session["Username"]), out int RequestId, out string Status, out string StatusMsg))
                    {
        ...
        protected void ProcessBCData_Click(object sender, EventArgs e)
        {
            if (HiddenField1.Value.ToString() == "Yes")
            {
                ...
                {
                    ScriptManager.RegisterStartupScript(this, typeof(Page), "Warning", "showWarning('Select Confirmation on all above BC Details are properly filled', 'Aggregator Registration');", true);
                    return;
                }
            }
            else
